...
unsigned short DStepSerial2::rxPin = GPIO_Pin_11;
unsigned short DStepSerial2::txPin = GPIO_Pin_10;
GPIO_TypeDef* DStepSerial2::rxGpioType = GPIOB;
GPIO_TypeDef* DStepSerial2::txGpioType = GPIOB;
USART_TypeDef* DStepSerial2::uartType = USART3;
...
void DStepSerial2::begin(int baudrate)
{
...
u8 DStepSerial2::read()
{
	return USART_ReceiveData(uartType);
}

void DStepSerial2::write(char ch)
{
	USART_SendData(uartType, (uint8_t) ch);
	while(USART_GetFlagStatus(uartType, USART_FLAG_TC) == RESET){}  
}
...
void DStepSerial2::print(char* string)
{
	while (0 != *string) {    
		USART_SendData(uartType, (uint8_t)*string);  
    string++;    
